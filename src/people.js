import { objects, getObjsArr, fireBullet, getBulletsArr, removeBullet, delta, countDeath } from './base.js'
...
export function mkPerson(x, y, opts={}) {
  ...
  const obj = Object.assign(p, {
    ...
    update() {
      ...
      /** @type {bullet} */
      let bulletHit
      if (obj != globalThis.p) obj.turn = Math.abs(obj.a) > 1.6 ? -1 : 1
      if (!obj.bless && (bulletHit = testBulletHit(obj))) { // Got a shot! Bad thing will happen.
        ...
        if (obj == player) return globalThis.pDeadCb()
        removeObj(obj)
        countDeath(bulletHit, obj)
        obj.die = 1
        obj.gun.t = 0
      ...
      obj.x += rebound.x//*1.5
      obj.y += rebound.y//*1.5
      if (!obj.die && !(enableCtrl && obj == globalThis.p)) {
...
function updateGun(p) {
  ...
  const timeout = p.we ? 200 : 300
  if (p.gun.t && p.gun.l < (Date.now()-timeout)) {
    p.gun.l = Date.now()
...
export function personAct(p) {
  if (p.dest) {
    if (
      delta(p.dest.x, p.x) < .3 &&
      delta(p.dest.y, p.y) < .3
    ) return p.vx = p.vy = 0
    p.a = Math.atan2(p.y-p.dest.y, p.dest.x-p.x)
  }
  if (p.panic) {
    p.v = .4
    p.a += Math.random()*2 - 1
  }
  p.vx = Math.cos(p.a) * p.v
  p.vy = -Math.sin(p.a) * p.v
  if (p.s) soldierAct(p)
}
...
function soldierAct(p) {
  ...
  if (!p.targ) {
    p.gun.t = 0
    p.gun.a = p.turn==1 ? (p.gun.a*8 + 1) / 9 : (p.gun.a*8 - 4) / 9
  } else {
    const gunY = p.y + p.h/2
    let angle = Math.atan2(gunY-p.targ.y, p.targ.x-p.x)
    if (delta(angle+twoPI, p.gun.a) < delta(angle, p.gun.a)) angle += twoPI
    if (delta(angle-twoPI, p.gun.a) < delta(angle, p.gun.a)) angle -= twoPI
...
export function testObjsColision(obj, other) {
  ...
  if (other.wall){
    if (other.dg) { // Object to Diagonal Wall colision
      ...
      const wallW = delta(other.x1, other.x2)
